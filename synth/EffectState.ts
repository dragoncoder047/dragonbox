...
export class EffectState {
    ...
    allocateNecessaryBuffers(synth: Synth, instrument: Instrument | AudioBus, effect: Effect, samplesPerTick: number): void {
    ...
    compute(synth: Synth, instrument: Instrument | AudioBus, effect: Effect, instrumentState: InstrumentState | AudioBusState, samplesPerTick: number, roundedSamplesPerTick: number, tone: Tone | null, channelIndex: number, instrumentIndex: number, envelopeStarts: number[], envelopeEnds: number[]): void {
        const samplesPerSecond: number = synth.samplesPerSecond;

        this.type = effect.type;

        const usesAudioBus = effect.type == EffectType.audioBus;
